...
constexpr int MAX_RECORDS = 500000;
...
std::string process_single_command(std::string_view line, FixedMiniDB& db) {
    ...
    if (cmd == "set" || cmd == "SET") {
        ...
        std::string_view key = get_next_token(line);
        std::string_view max_req_str = get_next_token(line);
        std::string_view ttl_sec_str = get_next_token(line);

        if(key.empty() || max_req_str.empty() || ttl_sec_str.empty()) return "ERR wrong number of arguments for 'limit'\r\n";
        if(key.size() > MAX_KEY_LEN) return "ERR key is too long\r\n";

        uint64_t max_req = 0;
        uint64_t ttl_sec = 0;

        auto [ptr, ec] = std::from_chars(max_req_str.data(), max_req_str.data() + max_req_str.size(), max_req);
        auto [ptr1, ec1] = std::from_chars(ttl_sec_str.data(), ttl_sec_str.data() + ttl_sec_str.size(), ttl_sec);
        if(ec != std::errc{} || ec1 != std::errc{}) return "ERR invalid syntax for 'limit'\r\n";

        try {
            std::string k(key);
            int64_t current = db.incr(k.c_str(), 1, ttl_sec);

            uint64_t retry_after_ms = db.get_exp(k.c_str());

            bool larger = current > max_req;

            int64_t remaining = larger == false ? max_req - current : 0;

            if(larger) {
                return "REJECT. {LIMIT: { " + std::string(max_req_str) + " }, RETRY-AFTER: { " + std::to_string(retry_after_ms / 1000) + " }, REMAINING: { " + std::to_string(remaining) + " }}\r\n";
            } else {
                return "ACCEPT. {LIMIT: { " + std::string(max_req_str) + " }, RETRY-AFTER: { " + std::to_string(retry_after_ms / 1000) + " }, REMAINING: { " + std::to_string(remaining) + " }}\r\n";
            }
